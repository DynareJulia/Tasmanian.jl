...
function checkDerivatives()
    ...
    grid1 = makeGlobalGrid(dimension = 2, outputs = 1, depth = 4, type = "iptotal", rule = "gauss-legendre")
    ...
    func2(x) = [2.0 * x[1] .* x[1] + x[2] .* x[2] / 2.0 + x[3] .* x[3], x[1] .* x[2] .* x[3]]
    grid2 = makeGlobalGrid(dimension = 3, outputs = 2, depth = 4, type = "iptotal", rule = "gauss-legendre")
