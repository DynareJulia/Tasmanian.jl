...
function checkAgainsKnown()
    ...
    grid = makeLocalPolynomialGrid(
        dimension = 2, outputs = 2, depth = 1, order = 1, rule = "localp")
    aT = zeros(2, 0)
    @test aT == getHierarchicalCoefficients(grid)
    grid = makeLocalPolynomialGrid(
        dimension = 2, outputs = 0, depth = 1, order = 1, rule = "localp")
    aT = zeros(0, 0)
    @test aT == getHierarchicalCoefficients(grid)
    grid = makeLocalPolynomialGrid(
        dimension = 2, outputs = 2, depth = 1, order = 1, rule = "localp")
    aPoints = getPoints(grid)
    aV = vcat(exp.(2.0 * aPoints[1, :] + aPoints[2, :])',
        sin.(3.0 * aPoints[1, :] + aPoints[2, :])')
    loadNeededPoints!(grid, aV)
    aS = getHierarchicalCoefficients(grid)
    aT = hcat([1.0, 0.0], [exp(-1.0) - 1.0, sin(-1.0)], [exp(1.0) - 1.0, sin(1.0)],
        [exp(-2.0) - 1.0, sin(-3.0)], [exp(2.0) - 1.0, sin(3.0)])
    @test aS ≈ aT

    grid = makeGlobalGrid(
        dimension = 3, outputs = 1, depth = 4, type = "level", rule = "fejer2")
    aPoints = getPoints(grid)
    aVan = evaluateHierarchicalFunctions(grid, aPoints)
    @test aVan ≈ I

    grid = makeGlobalGrid(
        dimension = 2, outputs = 1, depth = 1, type = "level", rule = "clenshaw-curtis")
    aPoints = hcat([0.33, 0.25], [-0.27, 0.39], [0.97, -0.76],
        [-0.44, 0.21], [-0.813, 0.03], [-0.666, 0.666])
    ...
    grid = makeLocalPolynomialGrid(
        dimension = 2, outputs = 1, depth = 4, order = 1, rule = "localp")
    aPoints = hcat([0.33, 0.25], [-0.27, 0.39], [0.97, -0.76],
        [-0.44, 0.21], [-0.813, 0.03], [-0.666, 0.666])
...
function checkSetCoeffsMergeRegine()
    ...
    gridA = makeGlobalGrid(
        dimension = 2, outputs = 1, depth = 4, type = "level", rule = "chebyshev")
    gridB = makeGlobalGrid(
        dimension = 2, outputs = 1, depth = 4, type = "level", rule = "chebyshev")
    loadExpN2!(gridA)
    ...
    gridA = makeGlobalGrid(
        dimension = 2, outputs = 1, depth = 4, type = "level", rule = "fejer2")
    gridB = makeGlobalGrid(
        dimension = 2, outputs = 1, depth = 4, type = "level", rule = "fejer2")
    loadExpN2!(gridA)
    setHierarchicalCoefficients!(gridB, getHierarchicalCoefficients(gridA))
    ...
    gridA = makeSequenceGrid(
        dimension = 2, outputs = 1, depth = 5, type = "ipcurved", rule = "min-delta")
    gridB = makeSequenceGrid(
        dimension = 2, outputs = 1, depth = 5, type = "ipcurved", rule = "min-delta")
    loadExpN2!(gridA)
    setHierarchicalCoefficients!(gridB, getHierarchicalCoefficients(gridA))
    compareGrids(gridA, gridB)
    ...
    gridA = makeLocalPolynomialGrid(
        dimension = 2, outputs = 1, depth = 4, order = 2, rule = "semi-localp")
    gridB = makeLocalPolynomialGrid(
        dimension = 2, outputs = 1, depth = 4, order = 2, rule = "semi-localp")
    loadExpN2!(gridA)
    setHierarchicalCoefficients!(gridB, getHierarchicalCoefficients(gridA))
    compareGrids(gridA, gridB)
...
function checkIntegrals()
    ...
    checkIntegrals(grid)
    grid = makeSequenceGrid(
        dimension = 2, outputs = 1, depth = 4, type = "level", rule = "rleja")
    checkIntegrals(grid)
...
function checkValues()
    ...
    grid = makeGlobalGrid(
        dimension = 2, outputs = 2, depth = 3, type = "level", rule = "clenshaw-curtis")
    grid = makeSequenceGrid(
        dimension = 2, outputs = 2, depth = 3, type = "level", rule = "rleja")
